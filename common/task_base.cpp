...
int task_base::open(size_t thread_num, size_t stack_size) {
    ...
    size_t i;
    pthread_attr_t attr;
    int t_ret = pthread_attr_init(&attr);
    if (t_ret) {
        return ret;
    }
    // create thread and wait execute together
    do {
        t_ret = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
        if (t_ret) {
            return ret;
        }
        if (t_ret = pthread_attr_setstacksize(&attr, stack_size))
            break;

        if (thread_num == 0 || (m_thread = (pthread_t*)malloc(thread_num * sizeof(pthread_t))) == NULL)
            break;

        pthread_barrier_init(&m_barrier, NULL, thread_num + 1); // here thread_num + 1 consist call activate thread
